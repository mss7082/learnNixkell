...
let
      ...
      let
        ...
        manual = _hfinal: hprev: {
          learnNixkell =
            ...
            in
            hprev.callCabal2nix "learnNixkell" filteredSrc { };
        };
      in
  ...
  ghc = ourHaskell.ghc.withPackages (_ps:
    pkgs.haskell.lib.getHaskellBuildInputs ourHaskell.learnNixkell
  );
...
in
{
  bin = util.leanPkg ourHaskell.learnNixkell;

  shell = pkgs.buildEnv {
    name = "learnNixkell-env";
